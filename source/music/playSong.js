...
module.exports = async (client, queue, song) => {
  try {
    ...
    let noMessage =
      require("../interactions/music/search").noMessage ||
      require("../commands/music/search").noMessage ||
      require("../interactions/music/play-playlist").noMessage ||
      require("../commands/music/play-playlist").noMessage;
    let getQueue;
    if (interaction) {
      try {
      ...
      if (noMessage !== "off") {
        if (lang == "en") {
          let embed = new MessageEmbed()
            .setColor("GREEN")
            .setFooter(
              song.formattedDuration + ` | ${song.likes}👍 ${song.dislikes}👎`
            )
          ...
          if (panelType == "buttons") {
            ...
            let row2 = new MessageActionRow().addComponents(btn4, btn5, btn6);
            let msg = await queue.textChannel.send({
              content: `**🔍 | :تم العثور على** \`${song.name}\`\n**تم التشغيل عن طريق: \`${song.user.username}\`**`,
              embeds: [embed],
              components: [row, row2],
            });
            const filter = (i) => i.user.id == song.user.id;
