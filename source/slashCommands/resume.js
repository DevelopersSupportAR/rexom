    ...
    run: async(client, interaction, args) => {
        try {
            ...
            if (lang == "en") {
                ...
                if (queue.paused == false) return embed.warn(interaction, "**The Music Is Not Paused**", "/");
                ...
                const queue = player.getQueue(interaction);
                if (!queue) return embed.notQueue(interaction, lang, "/");
                if (queue.paused == false) return embed.warn(interaction, "**لم يتم ايقاف الموسيى اصلا انت بتعمل ايه**", "/");
                player.resume(interaction);
