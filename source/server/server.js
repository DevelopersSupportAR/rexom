...
app.post("/dashboard/:guildID", checkAuth, async(req, res) => {
    ...
    if (req.body.role?.startsWith("<@&")) require('quick.db').set(`DJ_${req.params.guildID}`, req.body.role ? req.body.role.split('<@&')[1]?.split('>')[0] : "null")
  ...
  if (interaction && interaction.guild ?.id == req.params.guildID) {
    let queue = require("../client/player").getQueue(interaction);
